...
const FindSales = ({ onClose, onEdit, onReset, onBtnDelete, showTempMessage }) => {
    ...
    const loadQuot = async () => {
        try {
            const res = await axiosInstance.get(`${gapi}/sales/list`);
    ...
    const handleFilter = value => {
        ...
        const result = quot.filter(r =>
            (r.SalesId ?? '').toString().toLowerCase().includes(q) ||
                ...
                >
                    <table className="table table-bordered table-sm" style={{ fontSize: '12px', minWidth: '900px' }}>
                        <thead className="table-light">
                            <tr>
                                <th className="text-center" style={{ width: '60px' }}>Sales.No</th>
                                <th className="text-center" style={{ width: '80px' }}>Sales.Date</th>
                        ...
                        <tbody>
                            {filtered.length === 0 ? (
                                ...
                                <tr key={i} style={{ cursor: 'pointer' }}>
                                    <td className="text-center">{r.SalesId}</td>
                                    <td className="text-center">{r.SalesDate ? new Date(r.SalesDate).toLocaleDateString('en-GB') : ''}</td>
                                    ...
                                    <td className="text-center">
                                        <button className="btn btn-sm btn-secondary me-1"
                                            ...
                                            onClick={() => {
                                                console.log("Row SalesId 👉", r.SalesId);
                                                onEdit(r.SalesId);
                                            }}
                                        >🖋️Edit</button>
